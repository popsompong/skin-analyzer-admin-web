...
export function Sidebar() {
  ...
  return (
    <TooltipProvider delayDuration={120}>
      <aside
        className={cn(
          "sticky top-0 hidden h-screen shrink-0 border-r border-(--admin-sidebar-border) text-(--admin-sidebar-text) transition-[width] duration-150 ease-out md:flex md:flex-col",
          collapsed
      ...
      >
        ...
        >
          ...
          <SidebarSection collapsed={collapsed} items={contentItems} label="CONTENT" />
          <SidebarSection
            collapsed={collapsed}
...
function SidebarSection({ collapsed, items, label }: SidebarSectionProps) {
  ...
  if (collapsed) {
    return (
      <div aria-label={label} className="mt-3 space-y-1.5" role="group">
        ...
        />
        {items.map((item) => (
          <SidebarNavItem collapsed item={item} key={item.href} />
        ))}
      </div>
    );
  }
